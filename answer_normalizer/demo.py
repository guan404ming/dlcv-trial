...
def run_evaluation_ml(examples, model_path, verbose=False):
    """
    ...
    for i, ex in enumerate(examples, 1):
        ...
        try:
            ...
            predicted = ml_based_normalize_answer(
                answer, category, question, model_path
            )
            ...
            if verbose:
                ...
                print(
                    f"[ML] [{i}/{len(examples)}] {status} {category} ({elapsed_time:.2f}s)",
                    end="\r",
                )
...
def run_evaluation_lm(examples, verbose=False):
    ...
    for i, ex in enumerate(examples, 1):
        ...
        try:
            start_time = time.time()
            result = lm_based_normalize_answer(answer=answer, category=category)
            elapsed_time = time.time() - start_time
            ...
            if verbose:
                ...
                print(
                    ...
                    else f"Reasoning: {result['reasoning']}"
                )
                print(f"Time: {elapsed_time:.3f}s")
                print(f"Status: {'✓ CORRECT' if is_correct else '✗ WRONG'}")
            else:
                status = "✓" if is_correct else "✗"
                print(
                    f"[LM] [{i}/{len(examples)}] {status} {category} ({elapsed_time:.2f}s)",
                    end="\r",
                )
...
def print_metrics(results, method_name):
    ...
    if results["times"]:
        ...
        print(
            f"  Throughput: {len(results['times']) / results['total_time']:.2f} samples/sec"
        )
    ...
    for category in sorted(results["by_category"].keys()):
        ...
        print(
            f"  {status} {category:12s}: {cat_correct:3d}/{cat_total:3d} correct ({cat_accuracy:5.2f}%) | {cat_errors} errors"
        )
...
def print_comparison(ml_results, lm_results):
    ...
    ml_acc = (
        (ml_results["correct"] / ml_results["total"] * 100)
        if ml_results["total"] > 0
        else 0
    )
    lm_acc = (
        (lm_results["correct"] / lm_results["total"] * 100)
        if lm_results["total"] > 0
        else 0
    )
    ...
    print(
        f"  Winner: {'ML-based' if ml_acc > lm_acc else 'LM-based' if lm_acc > ml_acc else 'TIE'}"
    )
    ...
    if ml_results["times"] and lm_results["times"]:
        ...
        print(
            f"  Speedup: ML is {speedup:.2f}x {'faster' if speedup > 1 else 'slower'} than LM"
        )
        print(
            f"  ML throughput: {len(ml_results['times']) / ml_results['total_time']:.2f} samples/sec"
        )
        print(
            f"  LM throughput: {len(lm_results['times']) / lm_results['total_time']:.2f} samples/sec"
        )

    print("\nBy Category:")
    print("-" * 80)
    print(
        f"  {'Category':12s} | {'ML Accuracy':12s} | {'LM Accuracy':12s} | {'Winner':10s}"
    )
    print("-" * 80)

    all_categories = set(ml_results["by_category"].keys()) | set(
        lm_results["by_category"].keys()
    )
    for category in sorted(all_categories):
        ...
        ml_cat_acc = (
            (ml_stats["correct"] / ml_stats["total"] * 100)
            if ml_stats["total"] > 0
            else 0
        )
        lm_cat_acc = (
            (lm_stats["correct"] / lm_stats["total"] * 100)
            if lm_stats["total"] > 0
            else 0
        )
        ...
        winner = (
            "ML"
            if ml_cat_acc > lm_cat_acc
            else "LM"
            if lm_cat_acc > ml_cat_acc
            else "TIE"
        )

        print(
            f"  {category:12s} | {ml_cat_acc:10.2f}% | {lm_cat_acc:10.2f}% | {winner:10s}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test answer normalizer on validation data"
    )
    parser.add_argument(
        "--val_path",
        type=str,
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print detailed output for each example"
    )
    ...
    # Verify model path for ML method
    ...
    if args.method in ["ml", "all"]:
        ...
        if not (model_path / "best_model.pt").exists():
            ...
            print(
                "Skipping ML-based evaluation. Train the model first using ml/train.py"
            )
            if args.method == "ml":
                return
        else:
    ...
    if args.method in ["ml", "all"] and model_path_valid:
        print("\nRunning ML-based evaluation...")
        ml_results = run_evaluation_ml(examples, args.model_path, verbose=args.verbose)
